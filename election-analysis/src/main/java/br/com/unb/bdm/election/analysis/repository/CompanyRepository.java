...
public interface CompanyRepository extends GraphRepository<Company> {
	...
	@Query("MATCH (n)<-[r:DONATES_TO]-(c:Company) RETURN c as company, sum(r.value) as totalDonation ORDER BY totalDonation DESC LIMIT {limit}")
	List<CompanyLargestDonationData> loadLargestDonors(@Param("limit") int limit);

	@Query("MATCH (can:Candidate)<-[r:DONATES_TO]-(c:Company) RETURN c as company, sum(r.value) as totalDonation ORDER BY totalDonation DESC LIMIT {limit}")
	List<CompanyLargestDonationData> loadLargestDonorsCandidate(@Param("limit") int limit);

	@Query("MATCH (p:Party)<-[r:DONATES_TO]-(c:Company) RETURN c as company, sum(r.value) as totalDonation ORDER BY totalDonation DESC LIMIT {limit}")
